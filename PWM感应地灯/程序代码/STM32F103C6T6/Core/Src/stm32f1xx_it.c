...
void USART1_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART1_IRQn 1 */
	if((__HAL_UART_GET_FLAG(&huart1,UART_FLAG_IDLE) != RESET))
	{
		...
		xTaskNotifyFromISR( usart_debug_TaskHandle, 1U<<DEBUG_PARSE_DATA, eSetBits, &phpt );
		portYIELD_FROM_ISR( phpt );
		
	}else if ( __HAL_UART_GET_FLAG(&huart1,UART_FLAG_TC) != RESET ) {
		__HAL_UART_CLEAR_FLAG( &huart1, UART_FLAG_TC );
		xTaskNotifyFromISR( usart_debug_TaskHandle, 1U<<DEBUG_SEND_OK, eSetBits, &phpt );//DMA发送完成通知
		portYIELD_FROM_ISR( phpt );
	}
