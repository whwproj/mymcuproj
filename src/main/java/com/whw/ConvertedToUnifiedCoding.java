...
/**
 * 将多种编码格式的代码同一转换成一种指定编码格式
 * @author wuhanwei
 * @date 2022/06/30
 */
public class ConvertedToUnifiedCoding {
    ...
    private static void traversalDir(File dir) {
        ...
        try {
            sourcePath = dir.getCanonicalPath();
            targetPath = sourcePath.replace(sourceDirPath, targetDirPath);
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @param file 要转换的文件
     */
    private static void encodeConvert(File file) {
        ...
        try {
            //0.获取文件原始编码
            String encode = getJavaEncode(sourcePath);
            ...
            assert targetPath != null;
            OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(targetPath),  specifiedEncoding);
        ...
        System.out.println("输出: " + targetPath);
    }
