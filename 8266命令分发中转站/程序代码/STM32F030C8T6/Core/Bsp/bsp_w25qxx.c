...
static void W25Qx_Enable() {
	HAL_GPIO_WritePin(FLASH_CSN_GPIO_Port, FLASH_CSN_Pin, GPIO_PIN_RESET); // Chip select
}

//器件失能
static void W25Qx_Disable() {
	HAL_GPIO_WritePin(FLASH_CSN_GPIO_Port, FLASH_CSN_Pin, GPIO_PIN_SET); // Chip disselect
}
...
void read_data_from_w25qFlash( void ) {
	W25Qx_WAKEUP();
	W25Qx_Read_data((uint8_t*)&udata, STR_ADDR1, sizeof(USER_DATA));
	W25Qx_PowerDown();
	printf("w25qFlash read:\r\n");
	printf("wssid: %s\r\n", udata.wssid);
	printf("wpswd: %s\r\n", udata.wpswd);
	printf("tcpurl: %s\r\n", udata.tcpurl);
	printf("mqusername: %s\r\n", udata.mqusername);
