...
void debug_parse_data_fun( void ) {
	...
	if ( strstr( (char *)ds.rxBuff, "连接4G中" ) != NULL ) {
		...
		SPI_RW_Reg(NRF_WRITE_REG + STATUS, 0xf0, &hspi1 );//0xFF空指令
		printf("clear reg ok\r\n");
		...
		uint8_t sta = SPI_RW_Reg(NRF_READ_REG + STATUS, 0xff, &hspi1 );//0xFF空指令
		printf("sta: %02X\r\n", sta);
	...
	SPI_RW_Reg(NRF_WRITE_REG + STATUS, 0xf0, &hspi2 );//0xFF空指令
	printf("clear reg ok\r\n");
	...
	uint8_t sta = SPI_RW_Reg(NRF_READ_REG + STATUS, 0xff, &hspi2 );//0xFF空指令
	printf("sta: %02X\r\n", sta);
